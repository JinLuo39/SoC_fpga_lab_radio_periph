...
#define CLOCK_FREQUENCY 125000000 // system's clock frequency in Hz
...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 2048 * sizeof(unsigned int);//unsigned int is 4 bytes on my system
    unsigned int elapsed_clocks = stop_time - start_time;
    float time_spent = (float)elapsed_clocks / CLOCK_FREQUENCY; // Convert clock cycles to seconds
    throughput = ((float) bytes_transferred / (1024 * 1024)) / time_spent; // Convert bytes to Mbytes and divide by time in seconds   
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Jinyanzi(Jin) Luo - Custom Peripheral Demonstration\n\r");
    *(my_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 1; // make sure radio isn't in reset, my reset is active low
